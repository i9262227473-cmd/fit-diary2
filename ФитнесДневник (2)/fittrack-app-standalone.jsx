...
const TODAY_WORKOUT = {
  ...
  image: window.__resources.workoutChest,
  exercises: [
    { id: 1, name: 'Жим лёжа', muscle: 'Грудь', tag: 'Базовое', sets: 4, reps: '8-10', weight: 80,
      ...
      image: window.__resources.exerciseBench },
...
const NEXT_WORKOUT = { name: 'Ноги', when: 'Завтра', minutes: 60, image: window.__resources.workoutLegs };
...
function HomeScreen({ goTo, openWorkout }) {
  return (
    <div style={{padding:'8px 16px 16px',display:'flex',flexDirection:'column',gap:18}}>
      ...
      <div>
        ...
        <div style={{background:'var(--surface)',borderRadius:18,padding:'18px 18px 16px',display:'flex',flexDirection:'column',gap:14,position:'relative',overflow:'hidden'}}>
          ...
          <button onClick={openWorkout}
            style={{width:'100%',padding:'14px',background:'var(--accent)',color:'#000',border:'none',borderRadius:12,fontSize:14,fontWeight:700,letterSpacing:'0.04em',cursor:'pointer',textTransform:'uppercase',position:'relative',zIndex:2}}>
            НАЧАТЬ ТРЕНИРОВКУ
          </button>
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {PROGRESS_DATA.map(p => {
            const positive = p.trend.startsWith('+');
            const c = positive ? 'var(--accent)' : 'var(--accent)';
            return (
              <div key={p.name} style={{background:'var(--surface)',borderRadius:14,padding:'14px 16px',display:'flex',alignItems:'center',gap:14}}>
                ...
                <Spark points={p.points} color={c} w={90} h={36}/>
                <div style={{fontSize:13,fontWeight:700,color:c,minWidth:46,textAlign:'right'}}>{p.trend}</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{background:'rgba(61,220,92,0.08)',border:'1px solid rgba(61,220,92,0.25)',borderRadius:14,padding:'12px 14px',display:'flex',alignItems:'flex-start',gap:10}}>
        ...
        <button style={{background:'none',border:'none',color:'var(--text-dim)',cursor:'pointer',padding:0,marginTop:1}}><I.X c="var(--text-dim)" s={14}/></button>
      </div>
...
Object.assign(window, { HomeScreen, WorkoutScreen, ExerciseScreen, RestScreen, BottomNav, Spark, fmt, I, reducer, initialState, PROGRESS_DATA, FOOD_LOG, PROFILE, TODAY_WORKOUT });
