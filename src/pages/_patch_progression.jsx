          {plan.progression && plan.progression.increment_percent && (
            <div style={{ background:'#1a1a1a', borderRadius:18, padding:16, border:'1px solid #2e2e2e' }}>
              ...
              {[{ l:'Выполнил все подходы', v:`+${plan.progression.increment_percent?.min ?? 2.5}–${plan.progression.increment_percent?.max ?? 5}% к весу`, c:'#3d9970' }, { l:'RPE < 7 (легко)', v:'Увеличить нагрузку', c:'#3d9970' }, { l:'RPE 7–9 (норма)', v:'Оставить как есть', c:'#fbbf24' }, { l:'RPE > 9 (тяжело)', v:'Снизить нагрузку', c:'#f87171' }].map((row, i) => (
