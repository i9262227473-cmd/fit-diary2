...
export default function BarcodeScanner({ onDetect, onClose }) {
  ...
  // Сканер открывают кнопкой рядом с полем поиска продукта, у которого
  // autoFocus — клавиатура почти всегда уже открыта в этот момент. Портал
  // рендерится поверх, но фокус с поля никуда не девается, и мобильная
  // клавиатура не закрывается сама — из-за этого камера на весь экран
  // сжимается в верхнюю часть под клавиатурой. Снимаем фокус сразу при
  // монтировании сканера, чтобы клавиатура закрылась и камера встала на
  // весь экран, как задумано.
  useEffect(() => {
